...
namespace ConsoleApplication1
{
    class NPOItemplate
    {
        static void Main(string[] args)
        {
                ...
                var DataSet = DbHelperSQL.Query(@"select row_number() over(order by rowid asc) rowindex,module,partname,pn,moduleoriginalLevel,modulechangeToLevel,typename,moduleExcessWork,labours,repairs,materials,consumableMaterial,costs,percentageshow,curstatus ,'' remark from V_Dat_EWA_inquiryItemexport ");

                if (DataSet.Tables[0].Rows.Count > 0)
                {
                    var path = Environment.CurrentDirectory + @"\Template\Template.xlsx";
                    var Dic = new Dictionary<string, string>();
                    Dic["ewano"] = "asdasdasd";
                    IworkSave(GenerateIWorkbook(path, Dic, DataSet.Tables[0]),Environment.CurrentDirectory + @"..\..\..\OutPut\"+DateTime.Now.ToString("yyyyMMddhhmmss")+".xlsx");
                }
